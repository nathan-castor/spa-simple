...
function getCompanyDetails(tickerOrName,callback){
  ...
  request(url1, function(error, response, html){
    if(!error){
      ...
      company.name = $('h1').html().split('-')[0].trim()
      var data = $('tr');
      ...
      request(priceUrl, function (error, response, html) {
        if (!error && response.statusCode == 200) {
          ...
          if (isNaN(parseFloat(dataIndex[33]))) {
            company.beta = 1;
          }else{ 
...
module.exports = {
  ...
  //   var coName = companiesList[getAllCount]
  //   //console.log("company",company)
  ...
  //     if (getAllCount < companiesList.length) {
  //       getAllCount++
  //     }else{
  //       getAllCount = 0
  //       return
  ...
  updateAll: function(req,res){
    var coNameToSend = newCoList[updateAllCount]
    //console.log("company",company)
    getCompanyDetails(coNameToSend, function(company){
      //console.log("company",company)
      var updatedCompany = {
        companyName: company.name,
        symbol: company.symbol,
      ...
      console.log("updatedCompany.companyName",updatedCompany.companyName)
      if (updateAllCount < newCoList.length-1) { //
        updateAllCount++
      ...
      Stock.findOneAndUpdate({symbol: company.symbol}, updatedCompany, {new: true}, function(err, stock){
        if(err) return console.log(err)
        //POST THE STOCK
        console.log('stock.aveTGT:',stock.aveTGT)
        setTimeout(function() {
          request.patch("http://localhost:3000/user/scrape/stocks", function(error, response, html) {
            if(!error){
              //var rqstResponse = response
              console.log("response.body", response.body)
            }
    ...
    res.send("Should have updated STOCKS")
  },
  ...
  indexStockNames: function(req,res){
    var letterCount = 0;
    var validNames = []
    var inValidNames = []
    for (var i = 0; i < newCoList.length; i++) {
      var urlNames = 'https://www.stocktargetprices.com/company/'+newCoList[i];
      request(urlNames, function(error, response, html){
        if(!error){
          ...
          if (company.name != null) {
            validNames.push(comany.name)
          }else{
            inValidNames.push(comany.name)
          }
        }
      })
    }

    fs.writeFile("validNames.js", validNames, function(err) {
        if(err) {
            return console.log(err);
        }
        console.log("The file was saved!");
    }); 
    fs.writeFile("inValidNames.js", inValidNames, function(err) {
        if(err) {
            return console.log(err);
        }
        console.log("The file was saved!");
  ...
  //   var letterCount = 0;
  // 	var urlNames = 'https://www.stocktargetprices.com/search/company/list/'+Alphabet[letterCount];
  //   var stockNames = []
  //   scrape(urlNames, ['li a'], function (error, titles) {
  //       if(error) throw error;
  //       console.log('hacker news titles\n');
  //       titles.forEach(function (element, idx) {
  //         if (idx > 32) {
  //           console.log('  %d. %s', idx, element.html());
  //           stockNames.push(element.html().split('(')[0].replace(/&amp;/g, '&').replace(/&apos;/g, '\'').trim())
  //         }
  //       });
  //       fs.writeFile("stocks.js", stockNames, function(err) {
  //           if(err) {
  //               return console.log(err);
  //           }
  //           console.log("The file was saved!");
  //       }); 
			res.json(inValidNames)
  // 	})
  }
}
